...
export function Filter({ fields = [], onChange, onSubmit, hideProposers=false }) {
    ...
    return (
        <Form onSubmit={handleSubmit} onReset={handleReset}>
            <Row className="align-items-end">
                {fields.map((field) => (
                    <>
                        {(!hideProposers || field.id !== 'proposerId') && (
                            <Col md={3} key={field.id}>
                                <Form.Group className="mb-3">
                                    <Form.Label htmlFor={field.id} column={12}>{field.label}</Form.Label>
